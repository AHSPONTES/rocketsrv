...
#[get("/")]
fn index() -> &'static str {
    "Welcome to your own server, but there is nothing at the main branch\n"
}

#[get("/bacon")]
fn bacon() -> String {
    let bacon_contents = fs::read_to_string("bacon.txt").expect("Unable to open file");
...
fn ofage(name: String, age: u8) -> String {
    if age > 18 {
        format!(
            "Welcome, {}, your age {} means you can view this content!",
            name, age
        )
    } else {
        format!(
            "Sorry, {}, you are not the right age, since you are only {} years old!",
            name, age
        )
    }
}
